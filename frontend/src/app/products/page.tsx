...
//           : `fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products`)?category=${categoryFilter}`;
      ...
      try {
        ...
        const url =
          categoryFilter === 'All'
            ...
            : `${baseUrl}/api/products?category=${categoryFilter}`;
...
return (
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div className="flex flex-col md:flex-row gap-4 mb-10">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {['All', 'Furniture', 'Appliances', 'Electronics', 'Fitness'].map(
          cat => (
            <button
              key={cat}
              onClick={() => setCategoryFilter(cat)}
              className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-colors ${categoryFilter === cat
                  ? 'bg-indigo-600 text-white shadow-sm'
                  : 'bg-white border border-slate-200 hover:border-indigo-200 hover:text-indigo-700'
                }`}
            >
    ...
    {loading ? (
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 animate-pulse">
        {[...Array(10)].map((_, i) => (
          <div
            key={i}
            className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col h-80"
          >
            <div className="h-48 bg-slate-200 w-full" />
            <div className="p-4 sm:p-5 flex-grow flex flex-col gap-3">
              <div className="h-5 bg-slate-200 rounded-md w-3/4" />
